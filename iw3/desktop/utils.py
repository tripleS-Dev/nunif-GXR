...
def quality_to_crf(quality):
    ...
    max_crf = 35
    min_crf = 15
    return max(min_crf, min(max_crf, int(round(max_crf - (quality / 100.0) * (max_crf - min_crf)))))
...
def create_parser():
    ...
    parser.add_argument("--stream-quality", type=int, default=90,
                        help="Streaming quality (0-100, higher is better)")
    ...
    parser.add_argument("--gpu-jpeg", action="store_true", help="(Deprecated) No longer used")
...
def iw3_desktop_main(args, init_wxapp=True):
    ...
    if not args.local_viewer:
        ...
        server = StreamingServer(
            ...
            video_quality=quality_to_crf(args.stream_quality),
            auth=auth
        )
    else:
        # Local Viewer
    ...
    try:
        ...
        while True:
            with args.state["args_lock"]:
                ...
                if not args.local_viewer:
                    server.set_frame_data((sbs.detach().to("cpu").contiguous(), tick))
                else:
